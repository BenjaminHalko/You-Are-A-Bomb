/// @desc
draw_rectangle_color(0,0,room_width,global.bottom-136,#260000,#260000,c_black,c_black,false);
...
for(var i = 0; i < array_length(oBackground.b); i++) {
	if(oBackground.b[i].wait > 0) oBackground.b[i].wait--;
		...
		if(oBackground.b[i].y > global.bottom) {
...
if !surface_exists(surface) {
	...
	for(var i = 0; i < array_length(global.frontgrid); i++) draw_sprite_ext(sWallBackground,0,i*_size,room_height,1,global.frontgrid[i],0,merge_color(c_white,c_black,0.85),1);
	surface_reset_target();
}

draw_surface(surface,0,0);
gpu_set_blendmode(bm_subtract);
draw_rectangle_color(0,room_height/3*2+(room_height % TILE_SIZE),room_width,global.bottom,c_black,c_black,c_white,c_white,false);
